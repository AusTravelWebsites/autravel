...
export default function NewTripPage() {
  ...
  const submit = async () => {
    ...
    if (!destination.trim()) { setError('Please pick a location'); return; }
    ...
    try {
      const r = await fetch('/api/trips', {
        ...
        body: JSON.stringify({
          title: title.trim(),
          location_name: destination.trim(),
  ...
  return (
    <div style={{ minHeight:'100vh', background:C.bg }}>
      <div style={{ maxWidth:560, margin:'0 auto', padding:'32px 16px' }}>
        <button onClick={() => router.back()} style={{ background:'none', border:'none', color:C.sub, fontSize:14, cursor:'pointer', marginBottom:20, padding:0 }}>← Back</button>
        <div style={{ background:C.card, borderRadius:16, border:`1px solid ${C.border}`, padding:28 }}>
          <h1 style={{ fontSize:22, fontWeight:700, color:C.text, margin:'0 0 24px' }}>Plan an Adventure</h1>
          {error && (
            <div style={{ background:'#fef2f2', border:'1px solid #fecaca', borderRadius:8, padding:'10px 14px', color:C.red, fontSize:13, marginBottom:16 }}>{error}</div>
          )}
          <div style={{ display:'flex', flexDirection:'column', gap:18 }}>
            <div>
              <label style={label}>Title *</label>
              <input style={inp} placeholder="e.g. Southeast Asia 2026" value={title} onChange={e => setTitle(e.target.value)} />
            </div>
            <div>
            ...
            <div>
              ...
              <textarea style={{ ...inp, minHeight:90, resize:'vertical' as const }} placeholder="What's the plan?" value={description} onChange={e => setDescription(e.target.value)} />
            </div>
            ...
            <div>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:10 }}>
                ...
                <input style={{ ...inp, maxWidth:140 }} placeholder="Type or pick" value={coverEmoji} onChange={e => setCoverEmoji(e.target.value)} maxLength={8} />
                {coverEmoji && (
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(44px, 1fr))', gap:6, padding:10, background:'#f9fafb', borderRadius:10, border:`1px solid ${C.border}` }}>
                {['🌏','✈️','🏔️','🏖️','🏝️','🏕️','🗺️','🏛️','🍜','📸','🚴','🎒'].map(em => (
                  <button key={em} type="button" onClick={() => setCoverEmoji(em)} aria-label={`Use ${em}`}
                    style={{ fontSize:22, padding:6, background: coverEmoji === em ? C.tealLight : '#fff', border:`1px solid ${coverEmoji === em ? '#99f6e4' : C.border}`, borderRadius:8, cursor:'pointer', lineHeight:1, fontFamily:'inherit' }}>{em}</button>
                ))}
              </div>
            </div>
