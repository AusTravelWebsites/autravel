...
export default function NotificationsPage() {
  ...
  if (loading) return <div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'#ffffff'}}><div style={{width:40,height:40,border:'3px solid #333',borderTopColor:'var(--brand)',borderRadius:'50%',animation:'spin 0.8s linear infinite'}} /></div>
  return (
    <div style={{minHeight:'100vh',background:'#f3f4f6',color:'#111827',fontFamily:'system-ui'}}>
      ...
      <div style={{borderBottom:'1px solid #e5e7eb',position:'sticky',top:0,background:'#ffffff',zIndex:10}}>
        <div style={{maxWidth:600,margin:'0 auto',padding:'16px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
          <div style={{display:'flex',alignItems:'center',gap:12}}>
            ...
            {unread>0 && <span style={{background:'var(--brand)',color:'#f3f4f6',borderRadius:20,padding:'2px 8px',fontSize:12,fontWeight:700}}>{unread}</span>}
          </div>
      ...
      <div style={{maxWidth:600,margin:'0 auto',padding:16}}>
        ...
        {notifs.map((n:any) => {
            ...
            <>
              ...
              <div style={{flex:1}}>
                <div style={{fontSize:14,color:'#374151',lineHeight:1.5}}>
                  {n.actor_username && <span style={{color:'var(--brand)',fontWeight:600}}>{n.actor_name||n.actor_username}</span>}
          ...
          const styles: React.CSSProperties = {background:n.read?'#ffffff':'var(--brand-light)',border:`1px solid ${n.read?'#e5e7eb':'#f9fafb'}`,borderRadius:10,padding:'14px 16px',marginBottom:8,display:'flex',alignItems:'flex-start',gap:12,textDecoration:'none',color:'inherit'}
          return n.link
