...
export function DirectAffiliateCTA({ links, label }: { links: AffiliateLinks | null | undefined; label?: string }) {
  ...
  return (
    <section style={{ marginTop: 24, padding: '20px 22px', background: 'linear-gradient(135deg,#0d9488 0%,#065f46 100%)', borderRadius: 12, color: '#fff' }}>
      ...
      <div style={{ display: 'grid', gap: 10 }}>
        {entries.map(e => (
          <a key={e.key} href={e.url} rel="sponsored nofollow noopener" target="_blank"
             style={{ display: 'flex', justifyContent: 'space-between' as const, alignItems: 'center' as const, padding: '12px 16px', background: '#fff', color: '#065f46', borderRadius: 8, textDecoration: 'none', fontWeight: 700 }}>
